...
def kp_and_desc_from(algorithm_name: str, image: np.ndarray):
    ...
    #
        ...
        # fast.setNonmaxSuppression(0)test_tp3.py
...
def print_soluce_for_fragment(fragment_index: str):
    with open("C:\\Users\ddbtn\PycharmProjects\APP5_traitement-images\TP3\solution_fragments.txt", "r") as result_file:
...
def main():
    ...
    for fragment in fragments:
        ...
        # keep good matches
        for m0, m1 in pairwise(matches):
            x1, y1 = fragment_kp[m0.queryIdx].pt
            x2, y2 = target_kp[m0.trainIdx].pt

            x3, y3 = fragment_kp[m1.queryIdx].pt
            x4, y4 = target_kp[m1.trainIdx].pt
            if abs(get_euclidean_distance(x1, y1, x2, y2) - get_euclidean_distance(x3, y3, x4, y4)) < 20:
        ...
        for m in good_matches:
            for m1 in good_matches:
                ...
                size = original_fragment.shape[0]
        ...
        # while i < max_iter:
        #     m, m1 = np.random.choice(good_matches, 2, replace=True)
        #
        #     x1, y1 = fragment_kp[m1.queryIdx].pt
        #     x2, y2 = fragment_kp[m.queryIdx].pt
        #
        ...
        #
        #     size = original_fragment.shape[0]  # pareil pour width
        #     # coordonate base on the middle of the fragment
        #     x_frag = x1 - size // 2
        #     y_frag = y1 - size // 2
        #
        #     tx = int(float(x3 - (x_frag * np.cos(angle_diff) - x_frag * np.sin(angle_diff))))
...
if __name__ == '__main__':
    ...
    recontruct_image(sys.argv[1], sys.argv[2],
                     "reconstruction_results.txt", "result_euclidian.png", True)
